...
export interface DiagramExportOptions {
  ...
  /**
   * Fork addition — embed a 2× PNG instead of the SVG.
   *
   * Inline SVG is better everywhere it survives (crisp at any zoom, the labels
   * stay selectable text, a few kB), which is why every file export uses it.
   * Word, Google Docs and most mail clients drop inline `<svg>` on paste, so
   * "copy as HTML" asks for the raster instead of leaving a hole in the
    ...
    if (blocks.length) {
      ...
      for (const code of blocks) {
        ...
        try {
          ...
          if (opts.asPng) {
            // Fork addition: SVG → PNG, for surfaces that cannot take SVG.
            const png = await svgToPng(svg);
            if (!png) throw new Error('could not rasterize the diagram');
            const img = document.createElement('img');
            // Empty alt: the surrounding prose carries the meaning, and
            // inventing English alt text would leak into a translated note.
            img.alt = '';
            img.src = await pngDataUrl(png);
...
/**
 * Fork addition — the raster as a `data:` URL, for an `<img>` in the pasted
 * clipboard HTML (FileReader rather than `btoa`: a wide diagram's bytes would
 * blow the argument limit of `String.fromCharCode(...)`).
 */
async function pngDataUrl(png: DiagramPng): Promise<string> {
