...
| **Wikidata / Wikipedia entries** | Tried and lost. `Q140892885` (SoloMD), `Q140904452` (the developer item) and the Commons logo were created 2026-08-06 and **deleted 2026-08-07** — "Does not meet the notability policy". Both projects want the same thing first: two or three pieces of independent, reliable coverage. Re-creating before that reads as evading the deletion and risks getting the title salted. Earn the coverage, then revisit. |
...
- **2026-09-18** — **Wikidata entry deleted; external GEO assets need periodic re-verification.** The item, the developer item and the Commons logo were all removed the day after they were built, for notability. Nobody noticed for six weeks because nothing re-read them — our notes still said "live, verified". Two conclusions: (a) an entry on either wiki is blocked on independent coverage, not on effort, so it comes after press, not before; (b) anything we publish on a platform we do not control gets re-checked on a schedule, and with the platform's own API rather than a status code — `curl` returns 403 from AlternativeTo and Microsoft's store purely as bot defence, which had us believing an approved AlternativeTo listing was still pending for over a month.
- **2026-09-16** — **Startup time measured, and it was the bundle.** This doc asked for a measurement before inventing a "lightweight mode"; here it is. `vite.config.ts` carried no chunking strategy, so every static import landed in a 7.83 MB entry chunk that WebView2 compiles before the first paint — mermaid, the opencc + pinyin dictionaries (1.41 MB), html2pdf, docx, html2canvas, prettier, and both alternate root components. Deferring them halved the startup JS (7834 → 4026 KB) and cut first paint 29% cold / 32% warm in Chromium. No new mode required. Still eager and worth a second pass: katex and highlight.js (both in synchronous render paths) and the CodeMirror language packs.
